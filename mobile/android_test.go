...
func TestAndroid(t *testing.T) {
	...
	workspace, err := ioutil.TempDir("", "gabs-android-")
	if err != nil {
	...
	// Generate the mobile bindings for Gabs and add the tester class
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.ethereum", "github.com/abschain-develop/go-abs/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	build.CopyFile(filepath.Join("libs", "gabs.aar"), "gabs.aar", os.ModePerm)
