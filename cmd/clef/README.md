...
put together is a bit of a hack into the http server. This could probably be greatly improved
    - Relay: Gabs should be started in `geth --external_signer localhost:8550`.
    - Currently, the Gabs APIs use `common.Address` in the arguments to transaction submission (e.g `to` field). This
  ...
  retains the original input.
    - The Gabs api should switch to use the same type, and relay `to`-account verbatim to the external api.
...
The signer listens to HTTP requests on `rpcaddr`:`rpcport`, with the same JSONRPC standard as Gabs. The messages are
